...
export default async function YSPage() {
  ...
  const guides = allGuides.filter(guide => {
    // 确保guide是GuideData类型
    return typeof guide === 'object' && guide !== null && guide.game === "原神";
  });
            ...
            <div key={guide.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
              <div className="p-6">
                ...
                <p className="text-gray-600 mb-4 line-clamp-2">
                  这是一篇关于{guide.title}的详细攻略，包含最新的角色分析、武器评测和地图探索技巧...
                </p>
                <Link href={`/games/ys/guides/${guide.id}`} className="inline-block text-purple-600 font-medium hover:underline">
                  阅读全文 →
                </Link>
              </div>
            </div>
