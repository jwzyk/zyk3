...
export default async function JCCPage() {
  ...
  const guides = allGuides.filter((guide: any) => {
    return guide && typeof guide === 'object' && guide.game === "金铲铲之战";
  });
              ...
              <div key={guide.id || Math.random()} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                <div className="p-6">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="text-xs px-2 py-1 bg-purple-100 text-purple-800 rounded-full">{guide.category || '攻略'}</span>
                  ...
                  <p className="text-gray-600 mb-4 line-clamp-2">
                    这是一篇关于{guide.title || '游戏'}的详细攻略，包含最新的阵容推荐、英雄强度分析和羁绊效果解析...
                  </p>
                  <Link href={`/games/jcc/guides/${guide.id || '1'}`} className="inline-block text-purple-600 font-medium hover:underline">
                    阅读全文 →
                  </Link>
                </div>
              </div>
